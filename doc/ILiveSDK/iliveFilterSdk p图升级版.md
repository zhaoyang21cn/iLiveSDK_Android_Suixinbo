### iliveFilterSdk-1.2.2 p图升级版使用说明
使用前说明：
...
<pre>
static private class FrameFormat {
...
mTxcFilter.setListener(new TXIVideoPreprocessorListener(){
    @Override
    public void onProcessFrame(int texture, int width, int height, long ts) {
    ／／ 纹理id输出回调
    ...
    @Override
    public void onProcessFrame(byte[] data, int width, int height, int format, long ts) {
    ／／ 原始数据输出回调
